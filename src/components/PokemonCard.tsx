...
export const PokemonCard = ({ poke, boxNum }: { poke: Atom<Pokemon_V2_Pokemonspecies>, key: number, boxNum: number }) => {
    const [pokemon] = useAtom(poke);
    const [caught, setCaught] = useAtom(caughtStatus(pokemon.id));
    ...
    return (
        <Card className="aspect-square flex flex-col align-middle justify-between w-44 mt-3" onClick={handleCaught}>
            <CardHeader className='py-2 border-b-[1px] border-border border-solid'>
                <CardTitle className='w-full text-center'>{pokemon.name.charAt(0).toUpperCase()
                    + pokemon.name.slice(1)}</CardTitle>
            </CardHeader>
            <CardContent className='px-0 pb-0 flex justify-center'>
                <LazyLoadImage
