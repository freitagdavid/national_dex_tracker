...
export default function BoxLayout() {
    ...
    const filter = useAppSelector((state) => state.caught.pokemonFilter);
    const { data: pokemon, isLoading: pokemonLoading, isFetching: pokemonIsFetching } = useGetFilteredPokemonQuery(filter);
    // console.log(constructVariables([1, null, 1]));
    console.log(filter)

    if (pokemonLoading || pokemonIsFetching) {
        return (
